...
hp_boss = 100
hp_player = 100
print('周鹏血量:',hp_boss,'准备开始战斗！！'+'solex血量:',hp_player,'准备开始战斗！！')
while True:
    input_ni = int(input("输入数字1攻击！"))
    if input_ni == 1:
        # 表示solex打出的随机伤害，伤害值为10~20之间的数，包括10和20
        attack_player = random.randint(10,20)
        # Boss血量根据随机攻击值扣除
        hp_boss -= attack_player 
        print('你击中了周鹏，打出了',attack_player,'的伤害，周鹏剩余血量',hp_boss)
        if hp_boss > 0:   # 判断Boss是否已死，血量大于0说明活着，活着就会反击
        
            # Boss的随机反击伤害值
            attack_boss = random.randint(10,20)
            # solex扣血
            hp_player -= attack_boss
            print('愤怒的BOSS发动了攻击，对你造成了%s点伤害，你当前剩余血量%s' % (attack_boss,hp_player))
            # 判断solex是否已死
            if hp_player <= 0:
                print("好好的学不上，跑这里和人打架！120嘛，现场有人失血过多请派救护车到现场......")
                break
        else:
